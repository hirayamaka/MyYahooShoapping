...
class SearchItemTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    // APIを利用するためのクライアントID
    let appid = "dj00aiZpPTl0bDFUTHBHVk40SSZzPWNvbnN1bWVyc2VjcmV0Jng9OGQ-"
        ...
        let task = session.dataTask(with: request) { (data:Data?,
            ...
            guard error == nil else {
                ...
                DispatchQueue.main.async {
                    self.present(alert, animated: true, completion: nil)//メインスレッドに対して行う処理
                }
                return
            }
    ...
    //ToDo:お気に入りページをタップして次の画面に遷移する前の処理
   
    

    //価格安い順
    @IBAction func PriceAscendingOrder(_ sender: Any) {
        if(itemDataArray.count != 0){
            itemDataArray.sort(by: {Int($0.priceInfo.price!)! < Int($1.priceInfo.price!)!})
        }
        tableView.reloadData()
    }
    
    //価格安い順
    @IBAction func PriceDesendingOrder(_ sender: Any) {
        if(itemDataArray.count != 0){
            itemDataArray.sort(by: {Int($0.priceInfo.price!)! > Int($1.priceInfo.price!)!})
        }
        tableView.reloadData()
